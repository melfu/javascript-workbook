...
const sumEvens = () => {
    ...
    const sum = even.reduce(getSum, 0);
    sumEvens();
};
...
const atxIdx = (num) => {
    const valuePlusIndex = strNums.forEach = (num) => {
        num + strNums.indexOf(num);
    };
    valuePlusIndex();
    strNums.findIndex(valuePlusIndex === 512);
};
...
const weather = [{
    ...
    const weatherStates = weather.map(weatherStation => {
        return weatherStation.weather_state_name
    });
console.log(weatherStates);

//find the id of the object in weather that has a min_temp of 15.915

const idealTempObject = weather.filter((temp) => temp === 15.915);
const idealTemp = idealTempObject.weather_state_name;
